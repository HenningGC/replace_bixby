...
def download_data():
    ...
    AWSclients['s3'] = AWSClient(config=AWSClientConfig(
        service_name='s3',
    ...
    files = list_files_in_s3(AWSclients['s3'], bucket_name, 'instruction_audio/')
    ...
    for s3_file_name in files:
        ...
        download_from_s3(AWSclients['s3'], bucket_name, s3_file_name, output_file_name)
        
        # Extract utterances and save to local file
        extract_utterances(output_file_name)
